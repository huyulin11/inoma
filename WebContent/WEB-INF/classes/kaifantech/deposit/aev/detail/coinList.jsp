	...
	<c:forEach items="${detail}" var="detail" varStatus="status">
		<tr id="tr${detail.numindex}">
			...
			<td class='ipc2' class="bg">
				...
				name="list[${status.index}].shortsubject"
				value="${detail.shortsubject}" readonly autocomplete="off"/>
			</td>
			...
			<td class='ipc3' class="bg"><input type="hidden"
				name="list[${status.index}].zhongliang"
				value="${detail.zhongliang}" readonly autocomplete="off"/>
				<input type="text" value="${detail.weightshow}"/>
			</td>
			<td class='ipc' id="bg"><input type="text"
				name="list[${status.index}].zhiliang" value="${detail.zhiliang}"
				readonly autocomplete="off"/>
				
			</td>
			<td class='ipc'><input type="text"
				name="list[${status.index}].caizhi" value="${detail.caizhi}"
				readonly autocomplete="off"/>
			</td>
			<td class='ipc3'>
...
<c:if test="${detail.size()<=12 }">
	...
	<table style="font-size:11px; font-family:'黑体';width:720px;height:100px;margin:20px 40px 0px 40px;border-collapse: collapse;empty-cells: hide;">
	  <tr>
	    <td width="201">&nbsp;</td>
	    <td width="323" style="font-size:18px;">&nbsp;中国现代贵金属币/章送评申请表</td>	
	...
	<table style="font-size:11px; font-family:'黑体';width:720px; height:430px;margin:10px 40px;border:1px solid #6C655F;border-collapse: collapse;empty-cells: hide; float:left;">
	  <tr align="center" style="background-color:#DCDCDD;font-size:11px;">
	   <td width="25" style="border:1px solid #A0A0A0;">行号</td>
	    <td width="33" style="border:1px solid #A0A0A0;">年号</td>
	    <td width="176" style="border:1px solid #A0A0A0;">题材</td>
	    <td width="62" style="border:1px solid #A0A0A0;">重量</td>
	    <td width="59" style="border:1px solid #A0A0A0;">面值</td>
	    <td width="34" style="border:1px solid #A0A0A0;">精/普</td>
	    <td width="54" style="border:1px solid #A0A0A0;">材质</td>
	  	...
	  	<c:if test="${detail.size()<12 }">
		  	<c:forEach var="item" items="${detail}" varStatus="status">
		  		<tr align="center" style="font-size:11px;">
		  			...
		  			<td style="border:1px solid #A0A0A0;">${item.shortsubject}</td>
		  			<td style="border:1px solid #A0A0A0;">${item.zhongliang}</td>
		  			<td style="border:1px solid #A0A0A0;">${item.facevalue}</td>
		  			<td style="border:1px solid #A0A0A0;">${item.zhiliang}</td>
			    ...
			    <td width="70" style="border:1px solid #A0A0A0;">小计估价</td>
			  	...
			  	<c:forEach var="item" begin="0" end="11" items="${detail}" varStatus="status">
			  		<tr align="center" style="font-size:11px;">
			  			...
			  			<td style="border:1px solid #A0A0A0;">${item.years}</td>
			  			<td style="border:1px solid #A0A0A0;">${item.shortsubject}</td>
			  			<td style="border:1px solid #A0A0A0;">${item.weightshow}</td>
			  			<td style="border:1px solid #A0A0A0;">${item.facevalue}</td>
			  			<td style="border:1px solid #A0A0A0;">${item.zhiliang}</td>
			  			<td style="border:1px solid #A0A0A0;">${item.caizhi}</td>
			  			<td style="border:1px solid #A0A0A0;" align="right">${item.guestappraisal}</td>
