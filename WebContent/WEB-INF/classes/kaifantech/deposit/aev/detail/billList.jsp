...
<table id="tab4" class="tab4">
	<tr>
		...
		<th style="width: 40px;">年号</th>
		<th style="width: 80px;">国家/地区</th>
		<th style="width: 180px;">名称</th>
		<th style="width: 45px;">面值</th>
		<th style="width: 50px;">版别</th>
		<th style="width: 40px;">水印</th>
		<th style="width: 60px;">是否票样</th>
		<th style="width: 40px;">冠号</th>
		<th style="width: 60px;">单枚估价</th>
		<th style="width: 40px;">数量</th>	
		<th style="width: 60px;">是否连号</th>
		<th style="width: 70px;">小计估价</th>
		<th style="width: 80px;">小计评级费</th>
	</tr>
	<c:forEach items="${detail}" var="detail" varStatus="status">
		<tr id="tr${detail.numindex}">
			<td id="rowId">${status.index+1}</td>
			<td class='ipc' class="bg">
				<input type="text"
			...
			<td class='ipc'><input type="text"
				value="${detail.facevaluedis}" name="list[${status.index}].facevaluedis" readonly style="text-align:right;" autocomplete="off"/>
			</td>
			<td class="bg ipc"><input type="text"
				value="${detail.version}" name="list[${status.index}].version" readonly style="text-align:right;" autocomplete="off"/></td>
			<td class="bg ipc"><input type="text"
				value="${detail.wmk}" name="list[${status.index}].wmk" readonly style="text-align:right;" autocomplete="off"/></td>
			<td class="bg ipc">
				<c:if test="${detail.whetherSample=='1'}"><input type="text" value="是" readonly/></c:if>
				<c:if test="${detail.whetherSample=='0'}"><input type="text" value="否" readonly/></c:if>
			</td>
			<td class="bg ipc"><input type="text"
				value="${detail.blockNum}" name="list[${status.index}].blockNum" readonly style="text-align:right;" autocomplete="off"/></td>
			<td class='ipc3'>
			...
			<td class="bg ipc">
				<c:if test="${detail.whetherSeries=='1'}"><input type="text" value="是" readonly/></c:if>
				<c:if test="${detail.whetherSeries=='2'}"><input type="text" value="否" readonly/></c:if>
			</td>
...
<c:if test="${detail.size()<=12 }">
	...
	<table style="font-size:11px; font-family:'黑体';width:720px;height:100px;margin:20px 40px 0px 40px;border-collapse: collapse;empty-cells: hide;">
	  <tr>
	    <td width="201">&nbsp;</td>
	    <td width="323" style="font-size:18px;">&nbsp;纸钞送评申请表</td>	
	...
	<table style="font-size:11px; font-family:'黑体';width:720px;height:90px;margin:0px 30px 10px 40px;border:1px solid #6C655F;border-collapse: collapse;empty-cells: hide;">
	  <tr style="background-color:#DCDCDD;font-size:11px; font-family:'黑体';">
	    <td style="border-bottom:1px solid #A0A0A0;" colspan="6"><span style=" font-size:11px;">&nbsp;基&nbsp;本&nbsp;信&nbsp;息&nbsp;</span><span style="font-size:9px;">（注：每张评论表仅能选取一种评级类型和速度，请以正楷清晰书写）</span></td>
	  </tr>
	  <tr style="font-size:11px;">
	  	<td width="12%" style="border-bottom:1px solid #A0A0A0;">&nbsp;申请人：</td>
	  	<td width="19%" style="border-bottom:1px solid #A0A0A0;">${deposit.customer}</td>
	  	<td width="9%" style="border-bottom:1px solid #A0A0A0;">会员编号：</td>
	  	<td width="28%" style="border-bottom:1px solid #A0A0A0;">${deposit.usercode}</td>
	  	<td width="9%" style="border-bottom:1px solid #A0A0A0;">手机：</td>
	  	<td>${deposit.receivermobile}</td>
	  </tr>
	  <tr style="font-size:11px;">
	    <td>&nbsp;身份证号码：</td>
	    <td>610431199102322212</td>
	    <td>电子邮件：</td>
	    <td>${deposit.mail}</td>
	    <td>评级类型：</td>
	    <td>
	      <c:choose>
		  	<c:when test="${deposit.serviceid==1 }">
			    ...
			    <div style="float:left;margin-left: 10px;"><img src=""${ctx_images}/no_sel.jpg" width="8" height="8"/>重评</div>		
			    <div style="float:left;margin-left: 10px;"><img src=""${ctx_images}/no_sel.jpg" width="8" height="8"/>真品鉴定</div>		    
			</c:when>
				<c:when test="${deposit.serviceid==2 }">
			    <div style="float:left;">
			    <img src=""${ctx_images}/no_sel.jpg" width="8" height="8"/>新评</div>
			    <div style="width:10px; float:left;"><img src=""${ctx_images}/sel.jpg" width="8" height="8"/>重评</div>	
			    <div style="width:50px; float:left;"><img src=""${ctx_images}/no_sel.jpg" width="8" height="8"/>真品鉴定</div>		 	
			</c:when>
				<c:when test="${deposit.serviceid==5 }">
			    <div style="float:left;">
			    <img src=""${ctx_images}/no_sel.jpg" width="8" height="8"/>新评</div>
			    <div style="width:10px; float:left;"><img src=""${ctx_images}/no_sel.jpg" width="8" height="8"/>重评</div>		
			    <div style="width:10px; float:left;"><img src=""${ctx_images}/sel.jpg" width="8" height="8"/>真品鉴定</div>		 
			</c:when>
		</c:choose>
	    </td>
	  </tr>
	...
	<table style="font-size:11px; font-family:'黑体';width:720px;margin:20px 20px 0px 40px;height:150px;border:1px solid #6C655F;border-collapse: collapse;empty-cells: hide;">
	  <tr style="font-size:11px;">
	    <td style="background-color:#DCDCDD;width:48%; border-bottom:1px solid #A0A0A0;border:black solid 1px;">&nbsp;取&nbsp;件&nbsp;取&nbsp;件</td>
	 	<td rowspan="5" style="border:black solid 1px;"><img style="border:solid #DCDCDD 1px;float:right; margin-right:-1%;" src=""${ctx_images}/bill-baseprice.jpg?num=11"></td>
	  </tr>
  	  <tr style="font-size:11px;">
	...
	<table style="font-size:11px; font-family:'黑体';width:720px; height:430px;margin:10px 40px;border:1px solid #6C655F;border-collapse: collapse;empty-cells: hide; float:left;">
	  <tr align="center" style="background-color:#DCDCDD;font-size:11px;">
	   <td width="25" style="border:1px solid #A0A0A0;">行号</td>
	    <td width="40" style="border:1px solid #A0A0A0;">年号</td>
	    <td width="176" style="border:1px solid #A0A0A0;">名称</td>
	    <td width="62" style="border:1px solid #A0A0A0;">面值</td>
	    <td width="59" style="border:1px solid #A0A0A0;">版别</td>
	    <td width="34" style="border:1px solid #A0A0A0;">水印</td>
	    <td width="54" style="border:1px solid #A0A0A0;">冠号</td>
	    ...
	    <td width="70" style="border:1px solid #A0A0A0;">小计估价</td>
	  </tr>
	  	<c:if test="${detail.size()<12 }">
		  	<c:forEach var="item" items="${detail}" varStatus="status">
		  		<tr align="center" style="font-size:11px;">
		  			...
		  			<td style="border:1px solid #A0A0A0;">${item.years}</td>
		  			...
		  			<td style="border:1px solid #A0A0A0;">${item.wmk}</td>
		  			<td style="border:1px solid #A0A0A0;">${item.blockNum}</td>
	  ...
	  <c:choose>
	    ...
	    <td width="102" style="border-right:1px solid #A0A0A0;"><span style="letter-spacing:2px"><img src=""${ctx_images}/no_sel.jpg" width="8" height="8"/> 网络支付</span></td>
	  	...
	  	<c:when test="${deposit.payment==2}">
	  			<td width="10" align="center" style="border-bottom:1px solid #A0A0A0;"><img src="${ctx_images}/sel.jpg" width="8" height="8"/></td>
	    <td width="74" style="letter-spacing:1px"> 现金付款</td>
...
<c:if test="${detail.size()>12 }">
	<div id="printReqIIDiv" style="height:1050px;border:solid red 1px; width:720px; font-size:11px; font-family:'黑体';">
			...
			<table style="font-size:11px; font-family:'黑体';width:720px;margin:20px 20px 0px 40px;height:150px;border:1px solid #6C655F;border-collapse: collapse;empty-cells: hide;">
			  <tr  style="font-size:11px;">
			    <td style="background-color:#DCDCDD;width:48%; border-bottom:1px solid #A0A0A0;border:black solid 1px;">&nbsp;取&nbsp;件&nbsp;取&nbsp;件</td>
			 	<td rowspan="5" style="border:black solid 1px;"><img style="border:solid #DCDCDD 1px;float:right; margin-right:-1%;" src=""${ctx_images}/standard1.png"></td>
			  </tr>
		  	  <tr style="font-size:11px;">
			      <td style="border:1px solid #A0A0A0;">
				      <span style="margin-left:8px;"></span>取件方式：(单选)<span style="margin-left:10px;"><img src=""${ctx_images}/no_sel.jpg" width="8" height="8"/> 快递回邮   
			...
			<table style="font-size:11px; font-family:'黑体';width:720px; height:430px;margin:10px 40px;border:1px solid #6C655F;border-collapse: collapse;empty-cells: hide; float:left;">
			  <tr align="center" style="background-color:#DCDCDD;font-size:11px;">
			   <td style="border:1px solid #A0A0A0;" width="8%">行号</td>
			    <td width="40" style="border:1px solid #A0A0A0;">年号</td>
			    <td width="176" style="border:1px solid #A0A0A0;">名称</td>
			    <td width="62" style="border:1px solid #A0A0A0;">面值</td>
			    <td width="59" style="border:1px solid #A0A0A0;">版别</td>
		...
		<table style="font-size:11px; font-family:'黑体';width:720px; height:730px;margin:0px 40px;border:1px solid #6C655F;border-collapse: collapse;empty-cells: hide; float:left;">
		  <tr align="center" style="background-color:#DCDCDD;font-size:11px;">
		   <td width="25" style="border:1px solid #A0A0A0;" width="8%">行号</td>
		    <td width="40" style="border:1px solid #A0A0A0;">年号</td>
	  		<td width="176" style="border:1px solid #A0A0A0;">名称</td>
		    <td width="62" style="border:1px solid #A0A0A0;">面值</td>
		    <td width="59" style="border:1px solid #A0A0A0;">版别</td>
